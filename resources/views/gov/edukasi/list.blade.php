...
<style>
    ...
    @include('gov.edukasi.partials.header')
    </div>
    <div class="mx-4 mt-4 mb-4 grid grid-cols-[17%_auto] gap-x-4">
        @include('gov.edukasi.partials.aside')
                                    ...
                                    <div class="flex flex-row justify-between">
                                        <div>
                                            <div class="grid grid-cols-[50%_50%] mb-2">
                                                ...
                                                @endif
                                                <span
                                                    class="font-light ml-6">{{ \Carbon\Carbon::parse($item->created_at)->format('F Y') }}
                                                </span>
                                            </div>
                                            <p class="">{{ $item->isi }}</p>
                                        </div>
                        ...
                        <div class="p-4 md:p-5">
                            <form class="space-y-4" method="post"
                                ...
                                <div>
                                    ...
                                    <label for="judulEdu"
                                        class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Ulasan <span
                ...
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <div class="p-4 md:p-5">
                        <form class="space-y-4" enctype="multipart/form-data" method="post"
                            action="{{ route('pemerintah.edukasi.sub.edit', [$firstSub->id]) }}">
                            @csrf
                            @method('put')
                            <div>
                                <label for="judulEdu"
                                ...
                                <input type="text" name="judul" id="judul"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white"
                                    value="{{ old('judul', $firstSub->judul) }}" required />
                                <label for="judulEdu"
    ...
    {{-- @if (isset($firstSub->id)) --}}
        <div id="ratingModal" tabindex="-1" aria-hidden="true"
            class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-modal md:h-full">
            <div class="relative p-4 w-full max-w-md max-h-full">
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    <!-- Modal header -->
                    <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
                        <h3 class="text-xl font-semibold text-gray-900 dark:text-white">
                            Berikan Rating                        </h3>
                        <button data-modal-toggle="ratingModal" type="button"
                            class="end-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white"
                            data-modal-hide="authentication-modal">
                            <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none"
                                viewBox="0 0 14 14">
                    ...
                    <div class="p-4 md:p-5">
                        <form class="space-y-4" method="post"
                            action="{{ route('pemerintah.edukasi.rating', [$firstSub->id ?? 0]) }}">
                            @csrf
                            {{-- <div>                                 --}}
